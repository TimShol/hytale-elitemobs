...
public final class HealLeapAbilityComponent implements Component<EntityStore> {
    ...
    private static final KeyedCodec<Float> K_TRIGGER_HEALTH_PERCENT =
            new KeyedCodec<>("TriggerHealthPercent", new FloatCodec());
    private static final KeyedCodec<Long> K_COOLDOWN_TICKS_REMAINING =
            new KeyedCodec<>("CooldownTicksRemaining", new LongCodec());

    public static final BuilderCodec<HealLeapAbilityComponent> CODEC =
            BuilderCodec.builder(HealLeapAbilityComponent.class, HealLeapAbilityComponent::new)
                    .append(K_ABILITY_ENABLED, (c, v) -> c.abilityEnabled = v, c -> c.abilityEnabled).add()
                    .append(K_TRIGGER_HEALTH_PERCENT, (c, v) -> c.triggerHealthPercent = v, c -> c.triggerHealthPercent).add()
