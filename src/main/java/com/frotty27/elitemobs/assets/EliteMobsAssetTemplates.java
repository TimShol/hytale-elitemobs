...
public final class EliteMobsAssetTemplates extends LinkedHashMap<String, String> {
    ...
    /**
     * For abilities that need targeted keys for templates.
     */
    ...
    /**
     * For effects and single-template assets where the key is derived from the template filename.
     */
    ...
    private static @NonNull String getTemplateBaseName(int lastSlashIndex, String normalizedTemplatePath) {
        String fileName = (lastSlashIndex >= 0)
                ? normalizedTemplatePath.substring(lastSlashIndex + 1)
        ...
        return (lastUnderscoreIndex >= 0)
                ? fileName.substring(lastUnderscoreIndex + 1)
                : fileName;
    }
