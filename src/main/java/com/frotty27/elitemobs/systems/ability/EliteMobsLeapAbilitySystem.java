...
public final class EliteMobsLeapAbilitySystem extends EntityTickingSystem<EntityStore> {
    ...
    public EliteMobsLeapAbilitySystem(EliteMobsPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public void tick(
        ...
        EliteMobsConfig.AbilityConfig leapAbilityConfig =
                getAbilityConfig(config, ABILITY_CHARGE_LEAP_KEY);
        if (leapAbilityConfig == null || !AssetConfigHelpers.isTieredAssetConfigEnabledForTier(leapAbilityConfig, tierIndex)) {
            return;
        }
    ...
    private static EliteMobsConfig.AbilityConfig getAbilityConfig(EliteMobsConfig config, String abilityKey) {
        AssetConfigHelpers.getAssetConfig(config, AssetType.ABILITIES, abilityKey);
        return (EliteMobsConfig.AbilityConfig) AssetConfigHelpers.getAssetConfig(config, AssetType.ABILITIES, abilityKey);
    }
    ...
    private static boolean shouldLogGate(EliteMobsConfig config, EliteMobsLeapAbilityStateComponent abilityState) {
        ...
        if (!config.debug.isDebugModeEnabled) return false;
