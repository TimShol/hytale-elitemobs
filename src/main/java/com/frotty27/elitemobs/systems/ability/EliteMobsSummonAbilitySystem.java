...
public class EliteMobsSummonAbilitySystem extends EntityTickingSystem<EntityStore> {
    ...
    public EliteMobsSummonAbilitySystem(EliteMobsPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public void tick(float deltaTime, int entityIndex, @NonNull ArchetypeChunk<EntityStore> chunk, @NonNull Store<EntityStore> store, @NonNull CommandBuffer<EntityStore> commandBuffer) {
        ...
        if (summonConfig instanceof SummonAbilityConfig summonAbilityConfig) {
            ...
            if (transform != null) {
                ...
                plugin.getSummonRiseTracker().recordSummon(transform.getPosition(), AbilityConstants.SUMMON_MARKER_RANGE, roleNames);
            }
        }
        ...
        tierComponent.pendingSummonTick = currentTick + secondsToTicks(AbilityConstants.SUMMON_CHAIN_DELAY_SECONDS);
        commandBuffer.replaceComponent(npcRef, plugin.getEliteMobsComponent(), tierComponent);
