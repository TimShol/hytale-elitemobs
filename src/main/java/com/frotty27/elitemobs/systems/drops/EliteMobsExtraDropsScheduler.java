...
public final class EliteMobsExtraDropsScheduler {

    public record DropJob(
            long dueTick,
            Vector3d position,
            Vector3f rotation,
            ObjectArrayList<ItemStack> itemDrops,
            @Nullable String particleId
    ) {}

    private final TickClock tickClock;

    private final PriorityQueue<DropJob> scheduledDropJobs =
            new PriorityQueue<>(Comparator.comparingLong(DropJob::dueTick));

    public EliteMobsExtraDropsScheduler(TickClock tickClock) {
        this.tickClock = tickClock;
    }

    public void enqueueDrops(
            long delayTicks,
            Vector3d position,
            Vector3f rotation,
            ObjectArrayList<ItemStack> itemDrops,
        ...
        String normalizedParticleId =
                (particleId == null || particleId.isBlank()) ? null : particleId;

        synchronized (scheduledDropJobs) {
            scheduledDropJobs.add(
                    new DropJob(
                            dueTick,
                            position.clone(),
                            rotation.clone(),
    ...
    public void flushDue(Store<EntityStore> entityStore, CommandBuffer<EntityStore> commandBuffer) {
        ...
        while (true) {
            ...
            var itemEntities =
                    ItemComponent.generateItemDrops(entityStore, nextJob.itemDrops, nextJob.position, nextJob.rotation);
