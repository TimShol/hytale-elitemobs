...
public class ModelScalingSystem extends EntityTickingSystem<EntityStore> {
    ...
    public void tick(float deltaTime, int entityIndex, @NonNull ArchetypeChunk<EntityStore> chunk, @NonNull Store<EntityStore> store, @NonNull CommandBuffer<EntityStore> commandBuffer) {
        ...
        if (config.model.enableModelScaling && !tierComponent.scaledApplied) {
    ...
    private void applyModelScaleIfEnabled(
        ...
        if (!config.model.enableModelScaling) return;
        if (config.model.modelScaleMultiplierPerTier == null || config.model.modelScaleMultiplierPerTier.length <= tierComponent.tierIndex) return;
        ...
        boolean scaled = tryScaleModelComponent(
                ...
                commandBuffer,
                scaleMultiplier,
                config.debug.isDebugModeEnabled
        );
        if (!scaled) return;
    ...
    private float computeModelScaleMultiplier(EliteMobsConfig config, int tierIndex) {
        float baseMultiplier =
                (config.model.modelScaleMultiplierPerTier != null && config.model.modelScaleMultiplierPerTier.length > tierIndex)
                        ? config.model.modelScaleMultiplierPerTier[tierIndex]
                        : 1.0f;

        float variance = Math.max(0f, config.model.modelScaleRandomVariance);
