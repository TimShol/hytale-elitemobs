...
/**
 * Read-only query interface for inspecting the state of elite mobs.
 *
 * <p>Obtain an instance via {@link com.frotty27.elitemobs.api.EliteMobsAPI#query()}.
 * All methods accept an entity reference and return the requested data if the
 * entity is a recognized elite mob, or {@link Optional#empty()} / default values
 * otherwise.</p>
 *
 * @since 1.1.0
 */
public interface IEliteMobsQueryAPI {

    /**
     * Returns the tier index of the specified entity, if it is an elite mob.
     *
     * @param entityRef the entity reference to query
     * @return an {@link Optional} containing the zero-based tier index, or empty
     *         if the entity is not an elite mob
     */
    Optional<Integer> getTier(Ref<EntityStore> entityRef);

    /**
     * Returns whether the specified entity is an elite mob.
     *
     * @param entityRef the entity reference to query
     * @return {@code true} if the entity is an elite mob, {@code false} otherwise
     */
    boolean isEliteMob(Ref<EntityStore> entityRef);

    /**
     * Returns the distance-based health bonus applied to the specified elite mob.
     *
     ...
     *         entity is not an elite mob or has no distance scaling applied
     */
    Optional<Float> getDistanceHealthBonus(Ref<EntityStore> entityRef);

    /**
     * Returns the distance-based damage bonus applied to the specified elite mob.
     *
     * <p>This bonus is determined by how far the mob spawned from the world origin,
     * increasing difficulty in more remote areas.</p>
     *
    ...
    /**
     * Returns the distance from the world origin at which the specified elite mob spawned.
     *
     * @param entityRef the entity reference to query
     * @return an {@link Optional} containing the spawn distance, or empty if the
     *         entity is not an elite mob or the distance is not tracked
     */
    Optional<Float> getSpawnDistance(Ref<EntityStore> entityRef);

    /**
     * Returns the health multiplier applied to the specified elite mob.
     *
     ...
     *         the entity is not an elite mob or health scaling is not enabled
     */
    Optional<Float> getHealthMultiplier(Ref<EntityStore> entityRef);

    /**
     * Returns the damage multiplier applied to the specified elite mob.
     *
     ...
     *         the entity is not an elite mob or damage scaling is not enabled
     */
    Optional<Float> getDamageMultiplier(Ref<EntityStore> entityRef);

    /**
     * Returns the model scale factor applied to the specified elite mob.
     *
     ...
     *         entity is not an elite mob or model scaling is not enabled
     */
    Optional<Float> getModelScale(Ref<EntityStore> entityRef);

    /**
     * Returns whether the health of the specified elite mob has been finalized.
     *
     ...
     *         is not an elite mob or health has not yet been finalized
     */
    boolean isHealthFinalized(Ref<EntityStore> entityRef);

    /**
     * Returns the number of summoned minions currently active for the specified elite mob.
     *
     * @param entityRef the entity reference to query
     * @return an {@link Optional} containing the minion count, or empty if the
     *         entity is not an elite mob or does not track summoned minions
     */
    Optional<Integer> getSummonedMinionCount(Ref<EntityStore> entityRef);

    /**
     * Returns the entity reference of the last aggro target for the specified elite mob.
     *
     * @param entityRef the entity reference to query
     * @return an {@link Optional} containing the last aggro target reference, or
     *         empty if the entity is not an elite mob or has no recorded aggro target
     */
    Optional<Ref<EntityStore>> getLastAggroTarget(Ref<EntityStore> entityRef);

    /**
     * Returns the server tick at which the specified elite mob last acquired an aggro target.
     *
     * @param entityRef the entity reference to query
     * @return an {@link Optional} containing the tick number, or empty if the entity
     *         is not an elite mob or has no recorded aggro event
     */
    Optional<Long> getLastAggroTick(Ref<EntityStore> entityRef);

    /**
     * Returns whether the specified elite mob is currently in combat.
     *
     * <p>An elite mob is considered in combat when it has an active aggro target.</p>
     *
     * @param entityRef the entity reference to query
     * @return {@code true} if the elite mob is in combat, {@code false} otherwise
     */
    boolean isInCombat(Ref<EntityStore> entityRef);

    /**
     * Returns the migration version of the specified elite mob's component data.
     *
     * <p>Used internally to track which data migrations have been applied to the entity.</p>
     *
     * @param entityRef the entity reference to query
     * @return the migration version number, or {@code 0} if the entity is not an elite mob
     */
    int getMigrationVersion(Ref<EntityStore> entityRef);

    /**
     * Returns whether the specified elite mob needs a data migration.
     *
     * <p>An elite mob needs migration if its stored migration version is lower than
