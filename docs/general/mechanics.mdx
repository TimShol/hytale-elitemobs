...
The CAE is Hytale's built-in combat evaluation system. RPGMobs generates per-faction, per-tier, and per-weapon CAE configs at build time. This gives every elite realistic combat behavior without requiring a code-side state machine.
...
| Faction | Style | Attack Cooldown | Description |
| :--- | :--- | :--- | :--- |
| Skeleton | Disciplined | 1.0 - 2.0s | Consistent, measured attacks with predictable timing. |
| Trork | Berserker | 0.4 - 0.8s | Aggressive, fast-paced pressure with short cooldowns. |
| Outlander | Tactical | 0.8 - 1.2s | Balanced approach with flanking at T5. |
| Goblin | Chaotic | 0.3 - 2.0s | Erratic, unpredictable timing with a wide cooldown range. |
