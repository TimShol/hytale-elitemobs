...
`abilities.yml` defines the 10 combat abilities that make elites more than just stat-boosted mobs. Each ability has its own activation rules, cooldowns, per-tier scaling, and personality filtering.
...
A reactive and preemptive defensive ability. On damage received (all tiers), the elite has a chance to dodge sideways with a brief invulnerability window. T4+ elites with combat intelligence can also dodge preemptively when they detect their target is attacking or charging.
...
| Setting | What it does |
| :--- | :--- |
| `dodgeChancePerTier` | Probability that the elite dodges when triggered. T4+ intelligence boosts this by 2.5x for preemptive dodges. |
...
A staff-channeling AOE ability that buffs nearby allies and debuffs nearby enemies. The mob temporarily swaps to a configurable staff item during the cast. Available T3+ with Strategic and Berserker personalities.

```yaml
war_cry:
  isEnabled: true
  cooldownSecondsPerTier: [0, 0, 30, 25, 20]
  warCryTriggerChancePerTier: [0, 0, 0.08, 0.12, 0.18]
  npcStaffItemId: "Staff_Arcane"
```

| Setting | What it does |
| :--- | :--- |
| `warCryTriggerChancePerTier` | Probability of triggering per combat tick evaluation. |
| `npcStaffItemId` | The staff item the mob holds during the channeling animation. Configurable in the Admin UI via the item picker. |

## Enrage

A berserk transformation where the elite drops its weapon and enters a 20-second rapid punch chain. The mob cycles through 3 random variation patterns of light and heavy punches, then dies from exhaustion with normal loot and XP drops. Available T2+ with Aggressive, Tactical, Strategic, and Berserker personalities.
...
A ranged projectile burst for mobs wielding bows, crossbows, or guns. The mob aims upward briefly, then fires a spread of projectiles. Available T3+ with Aggressive and Tactical personalities.
...
## Personality-Based Ability Filtering

T4+ elites are assigned a combat personality that determines which abilities they receive. This creates coherent ability sets instead of every mob having every ability:

| Personality | Abilities | Style |
| :--- | :--- | :--- |
| Aggressive | Charge Leap, Multi Slash, Enrage, Dodge Roll, Volley | Gap-closing and sustained damage |
| Tactical | Charge Leap, Dodge Roll, Heal Leap, Volley, Multi Slash, Enrage | Balanced and adaptive |
| Strategic | War Cry, Undead Summon, Heal Leap, Dodge Roll, Enrage | Support and area control |
| Berserker | Multi Slash, Enrage, Charge Leap, Dodge Roll, War Cry | Relentless offense |

T1-T3 elites (without personalities) receive a limited set based on tier: T1 gets Dodge Roll only, T2 adds Multi Slash, T3 adds Enrage.
