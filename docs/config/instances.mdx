...
Below is an example `instances/Dungeon_Goblin.yml` overlay for a goblin dungeon instance:
...
This overlay sets progression to `NONE` (random tier assignment using the defined weights), increases health and damage scaling compared to the base config, disables friendly fire and fall damage for elites, and ensures nameplates and model scaling are active.
