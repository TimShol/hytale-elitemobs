...
if (result instanceof SpawnResult.Success s) {
    Ref<EntityStore> eliteRef = s.entityRef();
    int tier = s.tier();
    String role = s.roleName();
}
```
...
Pass a weapon category override to control what weapon the elite spawns with:
...
`SpawnResult` is a sealed interface. Failures carry a typed reason:

```java
if (result instanceof SpawnResult.Failure f) {
    switch (f.reason()) {
        case NO_MOB_RULE -> log("No mob rule for this role");
        case EVENT_CANCELLED -> log("A listener cancelled the spawn");
        case RPGMOBS_DISABLED_IN_WORLD -> log("RPGMobs is disabled here");
        default -> log("Failed: " + f.message());
    }
}
```
