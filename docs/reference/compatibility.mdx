...
EliteMobs can align its tier system with the external level system for consistent nameplate display if the compatible bridge plugin (EliteMobsRPGLevelingCompat) is detected. Without the bridge, a warning is shown to players on join.
