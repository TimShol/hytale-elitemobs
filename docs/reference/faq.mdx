...
1. Check `core.yml` → `enabledByDefault`. If set to `false`, the world needs a specific overlay file in `worlds/` or `instances/` with `enabled: true`.
...
Abilities are linked to specific mob rules with per-tier toggles. By default, most mob categories (Skeletons, Zombies, Goblins, Trorks, Outlanders, Wraiths) have abilities enabled for Tier 4 and 5. Additionally, each ability has weapon category restrictions — for example, Charge Leap is disabled for mobs with ranged weapons (staves, bows). Void mobs are excluded by default.
...
Particle effects come from the entity effects system in `effects.yml`. Set `isEnabled: false` on the `projectile_resistance` entry to disable both the effect and its particles.
