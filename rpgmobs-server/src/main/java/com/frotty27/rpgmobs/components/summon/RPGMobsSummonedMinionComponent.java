...
public final class RPGMobsSummonedMinionComponent implements Component<EntityStore> {
    ...
    public static final BuilderCodec<RPGMobsSummonedMinionComponent> CODEC = BuilderCodec.builder(
            RPGMobsSummonedMinionComponent.class,
            RPGMobsSummonedMinionComponent::new
    ).append(K_SUMMONER, (c, v) -> c.summonerId = v, c -> c.summonerId).add().append(K_MIN_TIER,
                                                                                     (c, v) -> c.minTierIndex = v,
                                                                                     c -> c.minTierIndex
    ).add().append(K_MAX_TIER, (c, v) -> c.maxTierIndex = v, c -> c.maxTierIndex).add().append(K_TIER_APPLIED,
                                                                                               (c, v) -> c.tierApplied = v,
                                                                                               c -> c.tierApplied
    ).add().append(K_CHAIN_DEATH_AT, (c, v) -> c.chainDeathAtTick = v, c -> c.chainDeathAtTick).add().build();

    @Override
    public Component<EntityStore> clone() {
        RPGMobsSummonedMinionComponent c = new RPGMobsSummonedMinionComponent();
