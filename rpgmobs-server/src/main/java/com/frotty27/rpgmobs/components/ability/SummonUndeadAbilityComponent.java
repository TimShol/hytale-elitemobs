...
public final class SummonUndeadAbilityComponent implements AbilityEnabledComponent, WeaponSwappable {
    ...
    private static final KeyedCodec<Long> K_PENDING_SUMMON_TICKS_REMAINING = new KeyedCodec<>(
            "PendingSummonTicksRemaining",
            new LongCodec()
    );
    private static final KeyedCodec<String> K_PENDING_SUMMON_ROLE = new KeyedCodec<>("PendingSummonRole",
                                                                                     new StringCodec()
    );

    public static final BuilderCodec<SummonUndeadAbilityComponent> CODEC = BuilderCodec.builder(
            SummonUndeadAbilityComponent.class,
            SummonUndeadAbilityComponent::new
    ...
    ).add().append(K_PENDING_SUMMON_TICKS_REMAINING,
                   (c, v) -> c.pendingSummonTicksRemaining = v,
                   c -> c.pendingSummonTicksRemaining
    ).add().append(K_PENDING_SUMMON_ROLE, (c, v) -> c.pendingSummonRole = v, c -> c.pendingSummonRole).add().build();
