...
public final class RPGMobsVanillaDropsCullZoneManager {

    private record CullZone(Vector3d centerPosition, double radiusSquared, long expiresAtTick) {
    }

    private final TickClock tickClock;
    private final CopyOnWriteArrayList<CullZone> activeCullZones = new CopyOnWriteArrayList<>();

    public RPGMobsVanillaDropsCullZoneManager(TickClock tickClock) {
    ...
    public void addCullZone(Vector3d centerPosition, double radius, long lifetimeTicks) {
        ...
        activeCullZones.add(new CullZone(centerPosition.clone(), radiusSquared, expirationTick));
    }
